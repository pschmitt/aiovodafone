...
async def connect(
    ...
    if device_type == DeviceType.TECHNICOLOR:
        api = VodafoneStationTechnicolorApi(hostname, username, password)
    elif device_type == DeviceType.SERCOMM:
        api = VodafoneStationSercommApi(hostname, username, password)
    else:
